...
def main():
  ...
  for path in arguments.inputs:
    if os.path.isdir(path):
      files.extend(glob.glob(os.path.join(path, arguments.pattern)));
    elif path.endswith(arguments.suffix) and os.path.isfile(path):
      files.append(path);
    else:
      print(f"[{now()}] duplicates.py: ignoring invalid input path {path}.",
  ...
  print("[{}] duplicates.py: {:,} documents from {:,} inputs; {:,} duplicates; {:.2f} seconds."
