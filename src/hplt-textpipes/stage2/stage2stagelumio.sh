#!/bin/bash
# Manual staging / pre-fetch of inputs to stage2 from LUMIO.
# stage2nodeparallel_batched.sh does staging inside, in parallel with processing. This script may be more
# efficient if one needs pre-fetching of many batches at once.
...
echo $XX $YY $ZZ | sed 's@lumio:@@g' | tr ' ' '\n' | \
  rclone copy -v lumio: $BASEOUTDIR --files-from="-" --transfers=4 --multi-thread-streams=0 --checkers=1 --no-traverse
