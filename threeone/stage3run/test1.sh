...
GLJOBS=$(( 3 * NJOBS / 5 )); (( GLJOBS < 1 )) && GLJOBS=1  
OLJOBS=$(( 2 * NJOBS / 5 )); (( OLJOBS < 1 )) && OLJOBS=1
...
#    jq -c '{lang:["cmn_Hans","cmn_Hant","jpn_Jpan"],prob:[0.9885,0.0115,0]}'
    ...
    parallel --halt now,fail=1 --block $LID_BLOCKSIZE -j "${1}" --pipe --keep-order  \
            "python -m hplt_textpipes.stage3.fastertext_lid.proto_langid --identity ${2} --text_field text"
    time_end=$(date +%s.%N)
...
#zstdcat "$INPUT_DIR/text.zst" | run_lid_parallel $(( 3 *  $GLJOBS / 2 )) glotlid-v3 >/dev/null
#zstdcat "$INPUT_DIR/text.zst" | run_lid_parallel $GLJOBS glotlid-v3 >"$FL1"


# Run xml2md in background and 1a in foreground: text.zst -> xml, text, md, htmllang
...
echo $(date +"%T") waiting for background xml2md processes
time -p wait $PID_MD 
...
# Run openlid, glotlid in background and 1b in foreground: text -> l1, l2
...
echo "$(date +"%T") All specified processes have finished."
